...
def encode_message(path,message, output=None):
	pixels = core.get_file_content(path)
	...
	if output != '' and output != False and output is not None:
		core.put_file_content(pixels, output, core.imsize(path))
...
def extract_message(image):
	if type(image) == str:
		image = core.get_file_content(image)
